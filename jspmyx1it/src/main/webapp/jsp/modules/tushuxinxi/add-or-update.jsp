...
<body>
					...
					<div class="row breadcrumb-bar">
						...
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>图书信息管理</span></li>
								<li class="breadcrumb-item active"><span>编辑图书信息</span></li>
							</ol>
						</div>
					</div>
				</div>
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>图书名称</label>
										<input name="tushumingcheng" id="tushumingcheng" name="tushumingcheng" class="form-control" placeholder="图书名称">
									</div>
									<div class="form-group col-md-6">
										<label>图书封面</label>
										<div><img id="tushufengmianImg" src="" width="100" height="100">
										<div class="upload">选择文件<input name="file" type="file" id="tushufengmianupload" class="form-control-file"></div>
										<input name="tushufengmian" id="tushufengmian-input" type="hidden"></div>
									</div>
									<div class="form-group col-md-6">
										<label>类型</label>
									...
									<div class="form-group col-md-6">
										<label>作者</label>
										<input name="zuozhe" id="zuozhe" name="zuozhe" class="form-control" placeholder="作者">
									</div>
									<div class="form-group col-md-6">
										<label>出版社</label>
									...
									<div class="form-group col-md-6">
										<label>图书简介</label>
										<textarea  name="tushujianjie" class="form-control" id="tushujianjie" rows="3"></textarea>
									</div>
									<div class="form-group col-md-6">
										<label>发布日期</label>
										<div id="faburiqiDate" class="input-append date form_datetime">
											...
											<div class="form-group  col-md-12" >
												<label>图书详情</label>
												<div><div class="upload"><input id="tushuxiangqingupload" name="file" type="file">选择文件</div>
												<script id="tushuxiangqingEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var tushuxiangqingUe = UE.getEditor('tushuxiangqingEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="tushuxiangqing" id="tushuxiangqing-input">
												</div>
											</div>	
	...
	<script>
		...
		function leixingSelect(){
			http("option/leixing/leixing","GET",{},(res)=>{
				if(res.code == 0){
		...
		// 文件上传
		function upload(){
			$('#tushufengmianupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {				
					document.getElementById('tushufengmian-input').setAttribute('value',"upload/"+data.result.file);
					if(document.getElementById('tushufengmianFileName') != null){
						document.getElementById('tushufengmianFileName').innerHTML = "上传成功!";
					}				
					$("#tushufengmianImg").attr("src",baseUrl+"upload/"+data.result.file);								
				}
			});
			$('#tushuxiangqingupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('tushuxiangqingEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.tushuxiangqing != null && ruleForm.tushuxiangqing != 'null' && ruleForm.tushuxiangqing != ''){
				var tushuxiangqingMes = '' + ruleForm.tushuxiangqing;
				var tushuxiangqingUeditor = UE.getEditor('tushuxiangqingEditor');
				tushuxiangqingUeditor.ready(function() {
					tushuxiangqingUeditor.setContent(tushuxiangqingMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('tushuxiangqingEditor').hasContents()){
				$('#tushuxiangqing-input').attr('value',UE.getEditor('tushuxiangqingEditor').getContent());
			}
		}
		...
		function dataBind(){
			...
			$("#tushufengmian-input").val(ruleForm.tushufengmian);
			...
			$("#tushuxiangqing").val(ruleForm.tushuxiangqing);	
			$("#faburiqi-input").val(ruleForm.faburiqi);
		}		
		//图片显示
		function showImg(){
			var tushufengmianFileName = "\"" + ruleForm.tushufengmian + "\"";
			$("#tushufengmianImg").attr("src",baseUrl+ruleForm.tushufengmian);
		}		
		//跨表
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.tushufengmian != null && res.data.tushufengmian != ''){

						$("#tushufengmianImg").attr("src",baseUrl+res.data.tushufengmian);
						$("#tushufengmian-input").val(res.data.tushufengmian);
					}
					...
					if(res.data != null && res.data != '' && res.data.tushuxiangqing != null && res.data.tushuxiangqing != ''){

						$("#tushuxiangqing").val(res.data.tushuxiangqing);
                        var tushuxiangqingMes = '' + res.data.tushuxiangqing;
                        var tushuxiangqingUeditor = UE.getEditor('tushuxiangqingEditor');
                        tushuxiangqingUeditor.ready(function() {
                            tushuxiangqingUeditor.setContent(tushuxiangqingMes);
                        });
