...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">图书信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>图书信息列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">图书信息列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<input type="text" id="weizhiSearch" class="form-control form-control-sm" placeholder="请输入位置" aria-controls="tableId">
									<label>
										作者
									</label>
									<input type="text" id="zuozheSearch" class="form-control form-control-sm" placeholder="请输入作者" aria-controls="tableId">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('zuozhe')">作者<i id="zuozheIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			if($('#weizhiSearch').val() != null && $('#weizhiSearch').val() != ''){
				searchForm.weizhi ="%" +  $('#weizhiSearch').val() + "%";
			}
			
			if($('#zuozheSearch').val() != null && $('#zuozheSearch').val() != ''){
				searchForm.zuozhe ="%" +  $('#zuozheSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("tushuxinxi/page","GET",{
				...
				weizhi : searchForm.weizhi,	
				zuozhe : searchForm.zuozhe,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var tushumingchengCell  = document.createElement('td');

						 tushumingchengCell.innerHTML = item.tushumingcheng;

			row.appendChild(tushumingchengCell);
			var tushufengmianCell  = document.createElement('td');

				                var tushufengmianImg = document.createElement('img');
			tushufengmianImg.setAttribute('src',baseUrl+item.tushufengmian);
			tushufengmianImg.setAttribute('height',100);
			tushufengmianImg.setAttribute('width',100);
			tushufengmianCell.appendChild(tushufengmianImg);


			row.appendChild(tushufengmianCell);
			...
			var zuozheCell  = document.createElement('td');

						 zuozheCell.innerHTML = item.zuozhe;

			row.appendChild(zuozheCell);
			var chubansheCell  = document.createElement('td');

						 chubansheCell.innerHTML = item.chubanshe;

			row.appendChild(chubansheCell);
			var tushujianjieCell  = document.createElement('td');

						  if(item.tushujianjie != null && item.tushujianjie != "" && item.tushujianjie.length >= 11){
			  	tushujianjieCell.innerHTML = item.tushujianjie.substring(0, 10) + "..."; 
			  }else{
			  	tushujianjieCell.innerHTML = item.tushujianjie;
			  }


			row.appendChild(tushujianjieCell);
			var tushuxiangqingCell  = document.createElement('td');
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "借书"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'jieshuxinxi','','','')");
			if(crossBtnControl('借书') == true){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("tushuxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"tushuxinxi");
